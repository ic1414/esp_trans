...
void setup() {
  ...
  bool circleValues = 1;
  rotaryEncoder.setBoundaries(0, sizeof(menu) - 1, circleValues);
...
int menu_nrf(int n, char which) {
  ...
  // pid
  static float pid[3] = {1.8, 0.4, 350.0};
  static float pidz[3] = {4.5, 0.6, 0.0};
  ...
  switch (n) {
    ...
    case 3:
      // nrf begin
      radioo.en = 0;
      for (int i = 0; i < 3; i++) radioo.rpid[i] = pid[i];
      for (int i = 0; i < 3; i++) radioo.rpidz[i] = pidz[i];
    ...
    case 4:
      ...
      */
      ...
      if (millis() - nrf_recieve_time > 500) {
        ack.motorOut[3] = 0;
        for (int i = 0; i < 3; i++)ack.cAngles[i] = 0.0;
        for (int i = 0; i < 3; i++) ack.voltages[i] = 0.0;
        for (int i = 0; i < 3; i++) ack.motorOut[i] = 0;
      }
      break;
